...
## 🚧 Active Project: Load Testing & Production Readiness

**Status:** In Progress (3-4 week timeline)
**Goal:** Evaluate system capacity for 2,500 concurrent users and write proposal

📋 **TODO Checklist:** See [TODO.md](TODO.md) for complete task list (26 tasks, Week 1 completed)
📖 **Detailed Plan:** See [Load Testing Plan](.claude/plans/graceful-swinging-hejlsberg.md) for implementation guide

**Quick Context:**
- Testing system's ability to handle 2,500 concurrent student registrations
- Using k6 (JavaScript-based load testing)
- Target: 10x performance improvement through 5 critical optimizations
- Deliverables: Technical brief (5-10 pages) + Presentation (15-20 slides)
...
The user hierarchy uses JPA Single Table Inheritance:
...
**Critical detail:** Use `instanceof` checks in service layer, NOT repository queries:
```java
...
**Why:** Single `users` table simplifies authentication; role differentiation via JPA inheritance.

**Files:** [User.java](backend/src/main/java/com/ems/backend/entity/User.java), [Student.java](backend/src/main/java/com/ems/backend/entity/Student.java), [EventOrganizer.java](backend/src/main/java/com/ems/backend/entity/EventOrganizer.java), [Administrator.java](backend/src/main/java/com/ems/backend/entity/Administrator.java)

### The Registration Gatekeeper Pattern

The most critical pattern for preventing race conditions during concurrent event registrations.

**File:** [RegistrationServiceImpl.java:39-114](backend/src/main/java/com/ems/backend/service/impl/RegistrationServiceImpl.java)

**Ordering (DO NOT change):**

1. **Validate** (idempotent checks: already registered, schedule conflict, capacity)
2. **Update Event count FIRST**
   ```java
   event.setCurrentRegistrations(currentRegistrations + 1);
   ```
3. **Force version check** (THE GATEKEEPER)
   ```java
   eventRepository.saveAndFlush(event); // Throws exception if another request was faster
   ```
4. **Create Registration record** (only if step 3 succeeds)
5. **Send confirmation email** (fire-and-forget)

**Why this order matters:**

If two users register simultaneously for the last spot:
- Both pass validation (capacity available)
- Both increment the counter
- First request's `saveAndFlush()` succeeds
- Second request's `saveAndFlush()` throws `ObjectOptimisticLockingFailureException` (version mismatch)
- Second request retries with fresh data, sees capacity reached, fails gracefully

**The registration record is ONLY created if the gatekeeper (step 3) allowed us through.**

This prevents ghost registrations and capacity overselling.
...
- ✅ User management (STI hierarchy: Student, EventOrganizer, Administrator)
- ✅ Proposal lifecycle (create, submit, approve/reject, resubmit with feedback)
- ✅ Event registration (with concurrency protection via optimistic locking)
- ✅ Event updates (admin approval workflow)
...
- ⚠️ Email notifications (interface exists, adapter incomplete)
- ⚠️ Audit logging (entity exists, no UI exposure)
...
- ❌ Production logging (structured logs, monitoring)
